...
export default function LoginPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Verificar si data.user no es null
      if (!data.user) {
        throw new Error("No se pudo obtener el usuario después del registro.")
      }

      // Inserta en la tabla users_profiles
      const { error: profileError } = await supabase
        .from('users_profiles')
        .insert([
          { user_id: data.user.id, full_name: registerName, address: '' } // Puedes agregar más campos según sea necesario
        ])
                ...
                <form onSubmit={handleLogin} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <LinkWithLoader href="/recuperar-contrasena" className="text-xs text-muted-foreground hover:text-primary">
